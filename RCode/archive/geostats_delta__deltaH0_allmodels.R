...
outfile<-paste("~/Code/Projects/CMIP5_p50/graphs/delta_allmodels_deltaH0_meanarea.ps")
postscript(outfile, height=5, width=5)
#quartz()
barplot(areatable_means, ylim=c(-5, 20), col=c("#7fc97f", "#beaed4", "#fdc086", "#ffff99", "#386cb0", "#f0027f"), xaxt="n", ylab=expression(paste("change in area with P50 depth (10"^"6", " km"^"2", ")", sep="")), beside=TRUE)
...
specieslist<-c("Thunnus_obesus", "Thunnus_albacares", "Katsuwonus_pelamis", "Thunnus_alalunga", "Thunnus_thynnus", "Thunnus_maccoyii")
  for(c in 1:length(specieslist)){
	file<-paste("/Data/Projects/CMIP5_p50/modelmean/modelmean.deltaH0.deltap50depth.", specieslist[c], ".nc", sep="")
	nc<-open.ncdf(paste(file, sep=""))	
...
outfile<-paste("~/Code/Projects/CMIP5_p50/graphs/delta_allmodels_deltaH0_meandepthchange.ps")
postscript(outfile, height=5, width=5)
par(las=1)
#quartz()

...
#-----------------------------------------
# Average Change in P50 Depth deltaH = 0
#-----------------------------------------
library(ncdf)
specieslist<-c("Thunnus_obesus", "Thunnus_albacares", "Katsuwonus_pelamis", "Thunnus_alalunga", "Thunnus_thynnus", "Thunnus_orientalis", "Thunnus_maccoyii")
  for(c in 1:length(specieslist)){
	file<-paste("/Data/Projects/CMIP5_p50/IUCN_modelmean/IUCN.modelmean.deltaH0.deltap50depth.", specieslist[c], ".nc", sep="")
	nc<-open.ncdf(paste(file, sep=""))	
		data<-get.var.ncdf(nc, nc$var[[1]], start=c(1,1), count=c(360, 180))
		close.ncdf(nc)
		data2<-as.vector(data)
...
locs<-seq(0.7,8.7,1.2)
vlocs<-c(1.3, 3.7)
boxplot(depthtable, col=collist, xaxt="n", ylab="change in depths (m)", ylim=c(-400, 400), at=locs, notch=TRUE, outline=FALSE)
axis(side=1, at=locs, labels=FALSE, tick=TRUE)
abline(h=0, lwd=1)
boxplot(depthtable, col=collist, xaxt="n", ylim=c(-400, 400), at=locs, notch=TRUE, outline=FALSE, add=TRUE)
...
text(-0.2, -380,"exothermic", cex=0.8, srt=60, pos=4)
text(1.8, -380,"independent", cex=0.8, srt=60, pos=4)
text(5.3, -380,"endothermic", cex=0.8, srt=60, pos=4)
mtext("(a)", side=3, at=0.2)
mtext("compression", side=4, las=3, at=200)
mtext("expansion", side=4, las=3, at=-200)

locs<-seq(0.7,8.7,1.2)
vlocs<-c(1.3, 3.7)
boxplot(depthtableH0, col=collist, xaxt="n", ylab="change in depths (m)", ylim=c(-400, 400), at=locs, notch=TRUE, outline=FALSE)
axis(side=1, at=locs, labels=FALSE, tick=TRUE)
abline(h=0, lwd=1)
boxplot(depthtableH0, col=collist, xaxt="n", ylim=c(-400, 400), at=locs, notch=TRUE, outline=FALSE, add=TRUE)
specieslist<-c(expression(italic("T. obesus")), expression(italic("T. albacares")), expression(italic("K. pelamis")), expression(italic("T. alalunga")), expression(italic("T. thynnus")), expression(italic("T. orientalis")), expression(italic("T. maccoyii")))

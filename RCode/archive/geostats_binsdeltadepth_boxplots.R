...
#quartz()
outfile<-paste("~/Code/Projects/CMIP5_p50/graphs/ESM2M_binsdeltadepth.ps")
...
for(a in 1:length(specieslist)){
	...
	folder<-paste("/Data/Projects/CMIP5_p50/ESM2M/", specieslist[a], "/deltap50depth_bins/", sep="")
	...
	for(b in 1:length(files)){
		...
		if(b==1){
			...
			colnames(depthtable)[b]<-paste(b*100, sep="")
		}	
	...
	boxplot(depthtable, main=specieslist[a], ylim=c(-350,350), notch=TRUE, outline=FALSE)
	abline(h=0)
}
